...
const HomePage = () => {
  ...
  const [hasShownWelcomeOnHome, setHasShownWelcomeOnHome] = useState(false);
  ...
  // Show welcome screen on home page after login
  useEffect(() => {
    if (isAuthenticated && user && !hasShownWelcomeOnHome) {
      console.log('HomePage - Showing welcome screen for authenticated user');
      const timer = setTimeout(() => {
        setShowWelcome(true);
        setHasShownWelcomeOnHome(true);
      }, 1000); // Small delay to ensure page is loaded

      return () => clearTimeout(timer);
    } else if (!isAuthenticated) {
      // Reset welcome screen state when user logs out
      setHasShownWelcomeOnHome(false);
      setShowWelcome(false);
    }
  }, [isAuthenticated, user, hasShownWelcomeOnHome]);
  ...
  const loadOffers = async () => {
    try {
      ...
      // Filter out loyalty reward offers
      const filteredHomepageOffers = homepageResponse.data.filter(offer =>
        !offer.title.toLowerCase().includes('loyalty reward')
      );
      const filteredPopupOffers = popupResponse.data.filter(offer =>
        !offer.title.toLowerCase().includes('loyalty reward')
      );

      setOffers(filteredHomepageOffers);
      setPopupOffers(filteredPopupOffers);

      console.log('Loaded homepage offers:', filteredHomepageOffers);
      console.log('Loaded popup offers:', filteredPopupOffers);
    } catch (error) {
