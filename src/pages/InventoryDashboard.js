...
const InventoryDashboard = () => {
  ...
  // Weekly Usage Comparison states
  const [weeklyData, setWeeklyData] = useState({
    ingredients: [],
    summary: { total_ingredients: 0, weeks_analyzed: 0 }
  });
  const [weeklyLoading, setWeeklyLoading] = useState(false);

  // Wastage & Unused Stock states
  const [wastageData, setWastageData] = useState({
    wastage: [],
    unused_stock: [],
    low_usage: [],
    summary: { total_waste_value: 0, total_waste_items: 0, unused_items_count: 0, low_usage_items_count: 0 }
  });
  const [wastageLoading, setWastageLoading] = useState(false);

  // Ingredient Breakdown states
  const [breakdownData, setBreakdownData] = useState({
    ingredients: [],
    summary: { total_ingredients: 0, total_value: 0, low_stock_count: 0, expiring_count: 0 }
  });
  ...
  }, [activeTab, selectedPeriod, breakdownCategory]);
  ...
  // Load Weekly Usage Comparison Data
  const loadWeeklyData = async () => {
    try {
      setWeeklyLoading(true);
      console.log('Loading weekly usage comparison data...');

      const response = await fetch('http://localhost:8000/api/inventory/ingredients/weekly_usage_comparison/?weeks=4', {
        headers: {
          'Authorization': `Token ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
        }
      ...
      if (response.ok) {
        ...
        console.error('Failed to load weekly data:', response.status);
        setError('Failed to load weekly usage comparison data');
      }
    } catch (error) {
      console.error('Error loading weekly data:', error);
      setError('Error loading weekly usage comparison data');
    } finally {
      setWeeklyLoading(false);
    }
  };

  // Load Wastage & Unused Stock Data
  const loadWastageData = async () => {
    try {
      setWastageLoading(true);
      console.log('Loading wastage & unused stock data...');

      const response = await fetch('http://localhost:8000/api/inventory/ingredients/wastage_unused_analysis/?period=month', {
        headers: {
          'Authorization': `Token ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
        }
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('Wastage data loaded:', data);
        setWastageData(data);
      } else {
        console.error('Failed to load wastage data:', response.status);
        setError('Failed to load wastage & unused stock data');
      }
    } catch (error) {
      console.error('Error loading wastage data:', error);
      setError('Error loading wastage & unused stock data');
    } finally {
      setWastageLoading(false);
    }
  };

  // Load Ingredient Breakdown Data
  const loadBreakdownData = async () => {
    try {
      setBreakdownLoading(true);
      console.log('Loading ingredient breakdown data...');

      const response = await fetch(`http://localhost:8000/api/inventory/ingredients/ingredient_breakdown/?category=${breakdownCategory}`, {
        headers: {
          'Authorization': `Token ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
        }
      ...
      if (response.ok) {
        ...
        console.error('Failed to load breakdown data:', response.status);
        setError('Failed to load ingredient breakdown data');
      }
    } catch (error) {
      console.error('Error loading breakdown data:', error);
      setError('Error loading ingredient breakdown data');
    } finally {
  ...
  const handleDeductStock = (ingredient) => {
    setDeductingIngredient(ingredient);
  ...
  const handleProcessDeduction = async (e) => {
    ...
    if (!deductionFormData.amount || parseFloat(deductionFormData.amount) <= 0) {
      alert('Please enter a valid deduction amount.');
      return;
    }

    if (parseFloat(deductionFormData.amount) > parseFloat(deductingIngredient.current_stock)) {
      alert(`Cannot deduct more than available stock (${deductingIngredient.current_stock} ${deductingIngredient.unit}).`);
      return;
    }

    try {
      const response = await fetch(`http://localhost:8000/api/inventory/ingredients/${deductingIngredient.id}/deduct_stock/`, {
        method: 'POST',
        headers: {
          'Authorization': `Token ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
        ...
        body: JSON.stringify({
          ...
          reason: deductionFormData.reason || 'Manual deduction'
        })
      });

      if (response.ok) {
        const result = await response.json();
        alert(`Successfully deducted ${result.deduction.amount} ${result.deduction.unit} from ${deductingIngredient.name}`);

        // Update the ingredient in the local state
        setIngredients(prev => prev.map(ing =>
          ing.id === deductingIngredient.id
            ? { ...ing, current_stock: result.deduction.new_stock }
            : ing
        ));

        // Close modal and reset form
        setShowDeductStock(false);
        setDeductingIngredient(null);
        setDeductionFormData({ amount: '', reason: '' });

        // Reload analytics data to reflect the change
        if (activeTab === 'others') {
          loadConsumptionData();
          loadWeeklyData();
          loadWastageData();
        ...
        const errorData = await response.json();
        alert(`Failed to deduct stock: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  const handleReorderIngredient = (ingredient) => {
    ...
    // Navigate to Purchase Orders tab instead of opening modal
  ...
  const handleDeleteIngredient = async (ingredientId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/inventory/ingredients/${ingredientId}/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Token ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok) {
        ...
        alert(`Failed to delete ingredient: ${JSON.stringify(errorData)}`);
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex items-center space-x-2">
                            ...
                            <button
                              ...
                              className="text-orange-600 hover:text-orange-900 text-xs px-2 py-1 rounded hover:bg-orange-50 flex items-center"
                              title="Deduct Stock"
                            >
        ...
        {activeTab === 'others' && (
          <div className="space-y-6">
            ...
            {!weeklyLoading && (
              <div className="bg-white rounded-lg shadow p-6">
                <h4 className="text-lg font-bold text-gray-900 mb-6">
                  Weekly Usage Comparison ({weeklyData.summary.weeks_analyzed} weeks)
                </h4>
                <div className="space-y-4">
                  {weeklyData.ingredients.map((ingredient, index) => {
                    const colors = ['#3B82F6', '#10B981', '#8B5CF6', '#F59E0B', '#EF4444', '#06B6D4', '#84CC16', '#F97316', '#EC4899', '#6366F1'];
                    const color = colors[index % colors.length];
                    const maxConsumption = Math.max(...weeklyData.ingredients.map(i => i.average_weekly_consumption));
                    const barWidth = (ingredient.average_weekly_consumption / maxConsumption) * 100;

                    return (
                      <div key={ingredient.ingredient_name} className="flex items-center">
                        <div className="w-24 text-sm font-medium text-gray-700 truncate">{ingredient.ingredient_name}</div>
                        <div className="flex-1 mx-4">
                          <div className="bg-gray-200 rounded-full h-6">
                            ...
                            >
                              <span className="text-white text-xs font-medium">{ingredient.average_weekly_consumption.toFixed(1)}</span>
                            </div>
                          </div>
                        </div>
                        <div className="w-16 text-sm text-gray-600">{ingredient.average_weekly_consumption.toFixed(1)} {ingredient.unit}/week</div>
                        <div className="w-20 text-sm text-right">
                          <span className={`font-medium ${ingredient.usage_percentage > 0 ? 'text-blue-600' : 'text-gray-500'}`}>
                            {ingredient.usage_percentage}%
                          </span>
                        </div>
                      </div>
                    );
            ...
            {!wastageLoading && (
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="mb-4 p-4 bg-red-50 rounded-lg">
                  <div className="flex items-center justify-between">
                    <span className="text-red-800 font-medium">Total Waste Value</span>
                    <span className="text-red-900 font-bold text-lg">Rs {wastageData.summary.total_waste_value.toFixed(2)}</span>
                  </div>
                  <div className="text-sm text-red-700 mt-1">
                    {wastageData.summary.total_waste_items} items wasted • {wastageData.summary.unused_items_count} unused items • {wastageData.summary.low_usage_items_count} low usage items
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-3">
                    <h5 className="font-semibold text-gray-700">High Wastage Items</h5>
                    {wastageData.wastage.length > 0 ? (
                      wastageData.wastage.map((item, index) => (
                        <div key={index} className="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                          <div>
                            <span className="text-gray-800 font-medium">{item.ingredient_name}</span>
                            <div className="text-sm text-gray-600">{item.total_wasted} {item.unit}</div>
                          </div>
                          <div className="text-right">
                            <span className="text-red-600 font-medium">Rs {item.waste_value.toFixed(2)}</span>
                            <div className="text-sm text-gray-500">{item.waste_count} incidents</div>
                          </div>
                        </div>
                      ))
                    ) : (
                  ...
                  <div className="space-y-3">
                    <h5 className="font-semibold text-gray-700">Unused Stock</h5>
                    {wastageData.unused_stock.length > 0 ? (
                      wastageData.unused_stock.map((item, index) => (
                        <div key={index} className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg">
                          <div>
                            <span className="text-gray-800 font-medium">{item.ingredient_name}</span>
                            <div className="text-sm text-gray-600">{item.current_stock} {item.unit}</div>
                          </div>
                          <div className="text-right">
                            <span className="text-yellow-600 font-medium">Rs {item.total_value.toFixed(2)}</span>
                            <div className="text-sm text-gray-500">{item.days_since_last_movement} days</div>
                          </div>
                        </div>
                      ))
                    ) : (
                      <div className="text-gray-500 text-sm">No unused stock</div>
                    )}
                  </div>
                  <div className="space-y-3">
                    <h5 className="font-semibold text-gray-700">Low Usage Items</h5>
                    {wastageData.low_usage.length > 0 ? (
                      wastageData.low_usage.map((item, index) => (
                        <div key={index} className="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
                          ...
                          <div className="text-right">
                            <span className="text-orange-600 font-medium">{item.consumption_percentage}%</span>
                            <div className="text-sm text-gray-500">Rs {item.total_value.toFixed(2)}</div>
                          </div>
                        </div>
                      ))
                    ) : (
            ...
            {!breakdownLoading && (
              <div className="bg-white rounded-lg shadow p-6">
                <div className="flex items-center justify-between mb-6">
                  <h4 className="text-lg font-bold text-gray-900">🔍 Ingredient Breakdown</h4>
                  <div className="flex space-x-2">
                    {[
                      ...
                      <button
                        key={category.key}
                        onClick={() => setBreakdownCategory(category.key)}
                        className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${breakdownCategory === category.key
                          ? 'bg-blue-600 text-white'
                ...
                <div className="mb-4 p-4 bg-blue-50 rounded-lg">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-900">{breakdownData.summary.total_ingredients}</div>
                ...
                <div className="space-y-3">
                  {breakdownData.ingredients.map((ingredient, index) => (
                    <div key={ingredient.ingredient_id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-600">{ingredient.description || 'No description'}</div>
                          <div className="text-sm text-gray-500">
                            {ingredient.current_stock} {ingredient.unit} • Rs {ingredient.unit_cost}/{ingredient.unit}
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-gray-900">Rs {ingredient.total_value.toFixed(2)}</div>
                        <div className={`text-sm ${ingredient.consumption_trend > 0 ? 'text-green-600' : ingredient.consumption_trend < 0 ? 'text-red-600' : 'text-gray-500'}`}>
                          {ingredient.consumption_trend > 0 ? '+' : ''}{ingredient.consumption_trend}% trend
                        </div>
                        <div className="text-sm text-gray-500">
                          {ingredient.days_remaining < 999 ? `${ingredient.days_remaining} days left` : 'No consumption data'}
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      {/* Deduct Stock Modal */}
      {showDeductStock && deductingIngredient && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold">Deduct Stock</h2>
              <button
                onClick={() => {
                  setShowDeductStock(false);
                  setDeductingIngredient(null);
            ...
            <form onSubmit={handleProcessDeduction} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Amount to Deduct ({deductingIngredient.unit}) *
                </label>
                <input
                  type="number"
                  step="0.01"
                  min="0.01"
                  max={deductingIngredient.current_stock}
                  value={deductionFormData.amount}
                  onChange={(e) => setDeductionFormData({
                    ...deductionFormData,
              ...
              <div>
                ...
                >
                  ...
                  <option value="Used for order">Used for order</option>
              ...
              <div className="flex justify-end space-x-4 pt-4">
                ...
                <button
                  ...
                  disabled={
                    ...
                    parseFloat(deductionFormData.amount) <= 0 ||
                    parseFloat(deductionFormData.amount) > deductingIngredient.current_stock ||
                    !deductionFormData.reason
                  }
                  className={`px-6 py-2 rounded-lg ${!deductionFormData.amount ||
                    parseFloat(deductionFormData.amount) <= 0 ||
                    parseFloat(deductionFormData.amount) > deductingIngredient.current_stock ||
                    !deductionFormData.reason
                    ? 'bg-gray-400 text-gray-200 cursor-not-allowed'
