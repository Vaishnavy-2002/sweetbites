      ...
      if (reportType === 'orders') {
        ...
        if (ordersData.recentOrders.length > 0) {
          ...
          ordersData.recentOrders.slice(0, 10).forEach(order => {
            const orderText = `Order #${order.order_number || 'N/A'} - ${order.customer_info?.display_name || order.customer?.username || 'Anonymous'} - ${order.order_status || 'N/A'} - RS ${order.total_amount || 0}`;
        ...
        if (feedbackData.recentFeedback.length > 0) {
          ...
          feedbackData.recentFeedback.slice(0, 10).forEach(feedback => {
            ...
            const feedbackText = `${feedback.user_info?.display_name || feedback.user_info?.username || feedback.user_info?.email || 'Anonymous'} - ${feedback.rating || 'N/A'} stars - ${shortComment}`;
                            ...
                            {reportsData.orders.recentOrders.map((order) => (
                              <tr key={order.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                  {order.customer_info?.display_name || order.customer?.username || 'Anonymous'}
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
