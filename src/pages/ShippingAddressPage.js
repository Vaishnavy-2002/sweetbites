...
const ShippingAddressPage = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      console.log('⚠️ User not authenticated, this is fine for guest users');
    }
  }, [cartItems, navigate]);
  ...
  const handleAddressSubmit = async (e) => {
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        console.log('Creating temporary address for guest:', tempAddress);

        // Add the temporary address to the current list
        setAddresses(prevAddresses => {
          const currentAddresses = Array.isArray(prevAddresses) ? prevAddresses : [];
          return [...currentAddresses, tempAddress];
        });

        // Select the newly created temporary address
  ...
  const handleProceedToConfirmation = () => {
    ...
    // Check if user has addresses or if we should create a temporary one
    let selectedAddress;
    ...
    if (addresses.length > 0 && selectedAddressId) {
      ...
      // Both guest and authenticated users must add an address
      console.log('❌ No addresses found - user must add an address');
      setError('Please add a shipping address to continue with your order.');
      return;
    }
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              {addresses.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-400 mb-4">Please add a shipping address to continue with your order.</p>
                  {!localStorage.getItem('token') ? (
                    <div className="mt-4">
                      <p className="text-sm text-blue-600 mb-2">You can add an address below without signing in!</p>
                    </div>
                  ) : (
                    <div className="mt-4">
                      <p className="text-sm text-blue-600 mb-2">You're signed in! Add your first address below.</p>
                    </div>
                  )}
                </div>
              )}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-md p-6 sticky top-4">
              ...
              <button
                ...
                className={`w-full py-3 px-4 rounded-lg font-medium transition-colors ${cartItems.length === 0 || addresses.length === 0
